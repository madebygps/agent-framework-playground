...
class SpamDetector(Executor):
    ...
    async def detect_spam(
        self, email: Email, ctx: WorkflowContext[Email]
    ) -> None:
        spam_keywords = ["buy now", "save big"]
        email.is_spam = any(
            keyword in email.body.lower() for keyword in spam_keywords
        )
...
class EmailProcessor(Executor):
    ...
    async def process_email(
        self, email: Email, ctx: WorkflowContext[Email, str]
    ) -> None:
...
class SpamHandler(Executor):
    ...
    async def handle_spam(
        ...
        await ctx.yield_output(
            f"{email.title} was detected as Spam and handled."
        )
...
async def main():
    ...
    builder.add_edge(
        spam_detector, email_processor, condition=lambda e: not e.is_spam
    )
    builder.add_edge(
        spam_detector, spam_handler, condition=lambda e: e.is_spam
    )
